...
if (isset($_GET['nome']) and isset($_GET['email']) and isset($_GET['cpf']) and isset($_GET['inst']) == isset($_GET[''])){
	...
	$emailRemetente = "TADS@IFSUL.EDU.BR";
	$assunto = "Confirmação de Inscrição - 1ª Semana Acadêmica do curso de Análise e Desenvolvimento de Sistemas- IFSUL";
	$mensagem = "   Olá,  $nome! 
	
		Este é um e-mail de confirmação de inscrição na 1° Semana Acadêmica do curso de Análise e Desenvolvimento de Sistemas que ocorrerá nos dias 18 à 20 de outubro de 2017 no Instituto Federal de Educação, Ciência e Tecnologia - IFSul câmpus Bagé.
		
		Confira em nosso site !--LINK DO SITE--! a programação, lembrando que as inscrições nos minicursos seram realizadas no dia durante o evento. Fique atento pois os minicursos teram vagas limitas. Mais informações em breve!
	...
	if(empty($nome)){
		$_SESSION['msg'] = "<script>alert('Preencha todos os campos para se cadastrar.'); history.back();</script>";
		mysqli_close($con);
		...
		header("Location: ../index.php");
			...
			if($query1 -> num_rows == 1 and $query2 -> num_rows == 1){
				...
				$_SESSION['msgcpfemail'] = "<script>alert('E-MAIL e CPF já cadastrado!!!'); history.back();</script>";
				mysqli_close($con);
				header("Location: ../index.php");
			...
			$salvar = mysqli_query($con, $sql);
				$_SESSION['msgcad'] = "<script>alert('Inscrição realizada com sucesso!!!'); history.back();</script>";
